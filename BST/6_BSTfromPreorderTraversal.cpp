...
public:
    TreeNode* BST(vector<int> &pre, int &index, int lower, int upper) {
        ...
        node->left = BST(pre, index, lower, node->val);
        node->right = BST(pre, index, node->val, upper);
        return node;
    }
...
// Helper function to perform an inorder traversal and print the BST
...
int main() {
    ...
    // Output the inorder traversal of the constructed BST
